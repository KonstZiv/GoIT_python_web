...
работать скрипт, распаковываются в папке с архивными файлами, при этом для каждого архива
...
def moves_files(sort_dict_full, path):
    ...
    def moves_files_kind(sort_dict_full, key):
    ...
    for key in sort_dict_full:
        if not (path / key).exists():
            (path / key).mkdir()
        threads[key] = Thread(target=moves_files_kind,
                              args=(sort_dict_full, key))
        threads[key].start()
    for key in threads:
        threads[key].join()
...
def unzip_archives(path):
    ...
    for elem in (path / 'archives').iterdir():
        ...
        if elem.suffix[1:] in available_arch_type:
            ...
            threads[elem] = Thread(target=shutil.unpack_archive, args=(
                elem, path / 'archives' / elem.stem))
            threads[elem].start()
...
def main():
    ...
    if input('для продолжения работы скрипта подтвердите действие (y/n): ') == 'y':
        ...
        if path.exists():
            if path.is_dir:
                ...
                sort_list_time_start = time.time()
                file_type_dict = creating_sort_list_in_dict(path, sort_dict)
                print(
                    f'время чтения имен файлов и сортировки их перед перемещением {time.time() - sort_list_time_start} секунд')
                # print(file_type_dict)
                start_moves = time.time()
                moves_files(file_type_dict, path)
                print(
                    f'время перемещения файлов в соотвествии со списком: {time.time() - start_moves} секунд')
                start_unzip = time.time()
                unzip_archives(path)
                print(
                    f'время разархивирования архивных файлов: {time.time() - start_unzip}')
                start_deleting_empty_folders = time.time()
                delete_empty_folder(file_type_dict, path)
                print(
                    f'время удаления пустых папок: {time.time() - start_deleting_empty_folders} секунд')
            else:
    ...
    print(f'общене время работы скрипта: {time.time() - start_time} секунд')
